    ...
    public partial class Lô_Thuốc : Form
    {
                ...
                if (dbDelete != null)
                {
                    if (MessageBox.Show("Bạn có chắc chắn muốn xóa dòng này!", "Thông Báo", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                    {
                        foreach (var tem in lst)
                            ...
                            {
                                if (tem.SOLUONG >= dbDelete.SOLUONG)
                                {
        ...
        private void Add_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (txtID.Text == "" || txtName.Text == "" || txtSL.Text == "")
                    throw new Exception("Vui lòng nhập đầy đủ thông tin");
                LOTHUOC dbUpdate = context.LOTHUOCs.FirstOrDefault(p => p.MALO == txtID.Text);
                THUOC dbUpdateT = context.THUOCs.FirstOrDefault(p => p.TENTHUOC == cmbMT.Text);
                if (dbUpdate != null)
                {
                    DialogResult dr = MessageBox.Show("Mã Lô Thuốc trùng. Chọn Yes để Cập Nhật", "Thông Báo", MessageBoxButtons.YesNo);
        ...
        {
            if (!char.IsLetter(e.KeyChar) && !char.IsNumber(e.KeyChar) & (Keys)e.KeyChar != Keys.Back & e.KeyChar != '.' & (Keys)e.KeyChar != Keys.Back & e.KeyChar != '-' & (Keys)e.KeyChar != Keys.Back & e.KeyChar != ' ')
            {
                e.Handled = true;
            }
        }
