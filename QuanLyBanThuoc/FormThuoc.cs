...
namespace QuanLyBanThuoc
{
    public partial class FormThuoc : Form
    {
        ...
        private void Add_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (txtID.Text == "" || txtName.Text == ""||txtGM.Text == "" || txtGB.Text == "")
                    throw new Exception("Vui lòng nhập đầy đủ thông tin");
                THUOC dbUpdate = context.THUOCs.FirstOrDefault(p => p.MATHUOC == txtID.Text);
                if (dbUpdate != null)
                {
                    DialogResult dr = MessageBox.Show("Mã Thuốc trùng. Chọn Yes để Cập Nhật", "Thông Báo", MessageBoxButtons.YesNo);
            ...
            try
            {
                ...
                if (dbDelete != null)
                {
                    if (MessageBox.Show("Bạn Có Muốn Xóa Thông Tin Này ?", "Thông Báo", MessageBoxButtons.YesNo) == DialogResult.Yes)
                    {
                        ...
                        foreach (var itam in listXK)
                            if (itam.MATHUOC == txtID.Text)
                            {
                                context.PHIEUXUATKHOes.Remove(itam);
                            }
                        foreach (var tem in listNK)
                            if (tem.MATHUOC == txtID.Text)
                            {
                                context.PHIEUNHAPKHOes.Remove(tem);
                            }
                        foreach (var tam in listDH)
                            if (tam.MATHUOC == txtID.Text)
                            {
                                context.PHIEUDATHANGs.Remove(tam);
                            }
                        foreach (var team in listLT)
                            if (team.MATHUOC == txtID.Text)
                            {
        ...
        private void cmbLoai_MouseDoubleClick(object sender, MouseEventArgs e)
        {
        ...
        {
            FormLoaiThuoc LT = new QuanLyBanThuoc.FormLoaiThuoc();
            LT.ShowDialog();
        }
    }
}
